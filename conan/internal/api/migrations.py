...
def update_file(file_path, new_content):
    ...
    Uses locking for settings.yml to prevent concurrent access races.
    ...
    # Use atomic creation to avoid TOCTOU race condition on first run
    if save_if_not_exists(file_path, new_content):
        return  # File was created, we're done

    # For settings.yml, acquire lock to prevent concurrent read/write races
    # Multiple processes may be migrating or reading settings simultaneously
    if file_name == "settings.yml":
        from conan.internal.cache.concurrency_lock import ConcurrencyLock
        # Extract home folder from file path (file_path is home_folder/settings.yml)
        home_folder = os.path.dirname(file_path)
        lock_manager = ConcurrencyLock(home_folder)
        with lock_manager.config_lock("settings.yml"):
            _update_file_locked(file_path, new_content, file_name, out)
    else:
        # Other files don't need locking (yet)
        _update_file_locked(file_path, new_content, file_name, out)


def _update_file_locked(file_path, new_content, file_name, out):
    """Helper function with the actual update logic, to be called with or without lock."""
