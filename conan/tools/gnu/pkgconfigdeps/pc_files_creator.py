...
def _get_components_pc_files_and_content(dep, components_info):
    """
...
def _get_package_with_components_pc_files_and_content(dep, package_info, components_info):
    """
    Get the PC files and content for dependencies with components
    """
    pc_files = {}
    pc_files.update(_get_components_pc_files_and_content(dep, components_info))
...
def _get_single_package_pc_file_and_content(dep, package_info):
    """
    Get the PC files for dependencies without components
    """
    description = "Conan package: %s" % package_info.name
    pc_file = get_pc_filename_and_content(dep, package_info.name,
                                          package_info.requires, description)
    return pc_file


def get_pc_files_and_content(dep):
    """
    ...
    if dep.cpp_info.has_components:  # Package with components
        ...
        pc_files.update(_get_package_with_components_pc_files_and_content(dep, pkg_info,
                                                                          components_info))
    else:
        # Package without components
        pkg_info = get_single_package_info(dep)
        components_info = []
        pc_files.update(_get_single_package_pc_file_and_content(dep, pkg_info))
