...
class XcodeDeps(object):
    ...
    def _vars_xconfig_file(self, dep, name, cpp_info):
        ...
        fields = {
            'name': name,
            'bin_dirs': " ".join('"{}"'.format(os.path.join(p)) for p in cpp_info.bindirs),
            'res_dirs': " ".join('"{}"'.format(os.path.join(p)) for p in cpp_info.resdirs),
            'include_dirs': " ".join('"{}"'.format(os.path.join(p)) for p in cpp_info.includedirs),
            'lib_dirs': " ".join('"{}"'.format(os.path.join(p)) for p in cpp_info.libdirs),
            'libs': " ".join("-l{}".format(lib) for lib in cpp_info.libs),
            'system_libs': " ".join("-l{}".format(sys_lib) for sys_lib in cpp_info.system_libs),
            'frameworksdirs': " ".join('"{}"'.format(os.path.join(p)) for p in cpp_info.frameworkdirs),
