...
VALID_LIB_EXTENSIONS = (".so", ".a", ".dylib")
...
class XcodeDeps(object):
    _vars_xconfig = textwrap.dedent("""\
        ...
        CONAN_{{name}}_DEPENDENCIES = "{{dependencies}}"
        {% if host_context %}
    ...
    _conf_xconfig = textwrap.dedent("""\
        ...
        FRAMEWORK_SEARCH_PATHS = $(inherited)$(CONAN_{{name}}_FRAMEWORK_PATHS)
    ...
    def _vars_xconfig_file(self, dep, name, cpp_info, deps, build=False):
        ...
        fields = {
            ...
            'system_libs': "".join([add_valid_ext(sys_dep) for sys_dep in cpp_info.frameworkdirs]),
    ...
    def _all_xconfig_file(self, name_general, deps):
        ...
        for req, dep in deps.items():
            ...
            if req.build:
                dep_name += "_build"
            conf_xconfig_name = "conan_%s.xconfig" % dep_name
            if conf_xconfig_name in content_multi:
                # already imported
                break
            else:
                content_multi = content_multi + '\n#include "conan_%s.xcconfig"\n' % dep_name
    ...
    def _content(self):
        ...
        for dep in host_req + test_req:
            ...
            vars_xconfig_name = "conan_%s_vars%s.xcconfig" % (dep_name, conf_name)
            result[vars_xconfig_name] = self._vars_xconfig_file(dep, dep_name, cpp_info, public_deps)
            props_name = "conan_%s%s.xcconfig" % (dep_name, conf_name)
            ...
            file_dep_name = "conan_%s.xcconfig" % dep_name
        ...
        for dep in build_req:
            ...
            # The entry point for each package, it will have conditionals to the others
            file_dep_name = "conan_%s.xcconfig" % dep_name
            dep_content = self._dep_xconfig_file(dep_name, file_dep_name, props_name, condition)
            result[file_dep_name] = dep_content
