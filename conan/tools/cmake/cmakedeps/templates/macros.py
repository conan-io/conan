...
class MacrosTemplate(CMakeDepsFileTemplate):
    ...
    def template(self):
       ...
       function(conan_package_library_targets libraries_vars_names package_libdir deps out_libraries_target config_suffix)
           ...
           foreach(_LIBRARY_VAR_NAME ${libraries_vars_names})
               if(NOT TARGET ${_LIBRARY_VAR_NAME})
                   ...
                   set(_LIB_TYPE ${${_LIBRARY_VAR_NAME}_TYPE})
                   set(_IMPORTED_LOCATION ${${_LIBRARY_VAR_NAME}_IMPORTED_LOCATION})
                   set(_IMPLIB_PATH ${${_LIBRARY_VAR_NAME}_IMPLIB_PATH})
                   add_library(${_LIBRARY_VAR_NAME} ${_LIB_TYPE} IMPORTED)
                   set_target_properties(${_LIBRARY_VAR_NAME} PROPERTIES IMPORTED_LOCATION ${_IMPORTED_LOCATION})
                   if(_IMPLIB_PATH)
                       set_target_properties(${_LIBRARY_VAR_NAME} PROPERTIES IMPORTED_IMPLIB ${_IMPLIB_PATH})
                   endif()
