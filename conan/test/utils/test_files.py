...
# Track fixture-owned temp folders for cleanup at process exit
_fixture_temp_folders = []
_fixture_cleanup_registered = False


def _cleanup_fixture_temp_folders():
    """Clean up fixture-owned temp folders at process exit."""
    for folder in _fixture_temp_folders:
        try:
...
def _register_fixture_temp_folder(folder):
    """Register a fixture-owned temp folder for cleanup at process exit."""
    global _fixture_cleanup_registered
    if not _fixture_cleanup_registered:
        atexit.register(_cleanup_fixture_temp_folders)
        _fixture_cleanup_registered = True
    _fixture_temp_folders.append(folder)


def temp_folder(path_with_spaces=True, create_dir=True, register_for_test_cleanup=True):
    ...
    # Always register for atexit cleanup
    # This avoids race conditions and getcwd() errors in parallel test execution
    # The register_for_test_cleanup parameter is kept for API compatibility
    _register_fixture_temp_folder(t)
