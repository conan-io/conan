...
from conan.internal.cache.conan_reference_layout import (EXPORT_SRC_FOLDER, EXPORT_FOLDER,
                                                         SRC_FOLDER, METADATA,
                                                         DOWNLOAD_EXPORT_FOLDER, RecipeLayout,
                                                         PackageLayout)
...
class CacheAPI:
    ...
    def restore(self, path) -> PackagesList:
        ...
        temp_extract_folder = cache.get_random_path()
        with open(path, mode='rb') as file_handler:
            ...
            the_tar.extractall(path=temp_extract_folder)
            the_tar.close()
        ...
        for ref, packages in package_list.items():
            ...
            extract_ref_layout = RecipeLayout(ref, os.path.join(temp_extract_folder,
                                                                ref_bundle["recipe_folder"]))
            try:
                extract_ref_layout = cache.create_ref_layout(ref, extract_ref_layout.base_folder)  # new DB folder entry
                out.info(f"Restore: {ref} in {extract_ref_layout.base_folder}")
            except ConanException:
                pass  # If restoring the previously existing one, to let it not fail

            for pref in packages:
                pref.timestamp = revision_timestamp_now()
                pref_bundle = package_list.package_dict(pref)
                pref_bundle["timestamp"] = pref.timestamp
                unzipped_pkg_folder = os.path.dirname(pref_bundle["package_folder"])
                extract_pkg_layout = PackageLayout(ref, os.path.join(temp_extract_folder,
                                                                     unzipped_pkg_folder))
                try:
                    pkg_layout = cache.create_pkg_layout(pref, extract_pkg_layout.base_folder)
                except ConanException:
                    pass  # DB Folder entry
                else:
                    out.info(f"Restore: {pref} in {pkg_layout.base_folder}")
                    pref_bundle["package_folder"] = pkg_layout.package()
                    unzipped_metadata_folder = pref_bundle.get("metadata_folder")
                    if unzipped_metadata_folder:
                        pref_bundle["metadata_folder"] = pkg_layout.metadata()
