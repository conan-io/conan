...
class TestLibsComponents:
    ...
    def test_libs_no_components_multilib_component(self):
        """
        cpp_info.libs = ["lib1", "lib2"]
        """
        c = TestClient()

        base_h = gen_function_h(name="base")
        base_cpp = gen_function_cpp(name="base", includes=["base"])
        ...
        """)
        base_cmakelists = textwrap.dedent("""
            set(CMAKE_CXX_COMPILER_WORKS 1)
            set(CMAKE_CXX_ABI_COMPILED 1)
            cmake_minimum_required(VERSION 3.15)
            project(base CXX)
            ...
            set_target_properties(base PROPERTIES PUBLIC_HEADER "src/base.h")
            install(TARGETS base)
        """)

        vector_h = gen_function_h(name="vector", includes=["base"])
        vector_cpp = gen_function_cpp(name="vector", includes=["vector"])
        module_h = gen_function_h(name="module")
        module_cpp = gen_function_cpp(name="module", includes=["module"])
        ...
        conanfile = textwrap.dedent("""
            ...
            class Matrix(ConanFile):
                ...
                def requirements(self):
                    self.requires("base/1.0", transitive_headers=True)
                ...
                def package_info(self):
                    self.cpp_info.set_property("cmake_target_name", "matrix::matrix")
                    self.cpp_info.libs = ["module", "vector"]
                  """)
        ...
        cmakelists = textwrap.dedent("""
            ...
            target_link_libraries(vector PRIVATE base::base)

            set_target_properties(vector PROPERTIES PUBLIC_HEADER "src/vector.h")
            set_target_properties(module PROPERTIES PUBLIC_HEADER "src/module.h")
            install(TARGETS module vector)
            """)
        c.save({"base/conanfile.py": base_conanfile,
                "base/CMakelists.txt": base_cmakelists,
                "base/src/base.h": base_h,
                "base/src/base.cpp": base_cpp,
        ...
        c.run("new cmake_lib -d name=app -d version=0.1 -d requires=matrix/1.0 -o=app")
        app_cpp = textwrap.dedent("""
            #include <iostream>
            #include "vector.h"
            #include "module.h"
            ...
            void app(){
                std::cout << "app/0.1 - Ran app()" << std::endl;
                vector();
                module();
            }
            ...
            void app_print_vector(const std::vector<std::string> &strings) {
                for(std::vector<std::string>::const_iterator it = strings.begin(); it != strings.end(); ++it) {
                    std::cout << "app/0.1 " << *it << std::endl;
                }
            }
        """)

        c.save({"app/src/app.cpp": app_cpp})
        c.run(f"create app -c tools.cmake.cmakedeps:new={new_value}")
