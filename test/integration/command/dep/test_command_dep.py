...
def test_add_dep():
    ...
    client.run("dep add -req hello", assert_error=True)
    ...
    client.run("dep add --requires=hello")
    ...
    client.run("dep add --requires=hello --tool-requires=hello --test-requires=hello")
    ...
    client.run("dep add --requires=bye --no-remote")  # from cache
    ...
    client.run("dep add --requires=mylib/1.2")
    assert "Added 'mylib/[>=1.2 <2]' as a new requires." in client.out

    # Using commit as a version
    client.run("dep add --requires=other/cci.20203034")  # can not bump the version, won't use vrange
...
def test_remove_dep():
    ...
    # No conanfile is present - error
    client.run("dep remove --requires=hello", assert_error=True)
    assert "ERROR: Conanfile not found at" in client.out
    client.save({"conanfile.py": GenConanfile(name="app")})
    # No requirement "hello" declared
    client.run("dep remove --requires=hello")
    ...
    client.run("dep remove --requires=hello --tool-requires=hello --test-requires=hello")
