...
class TestDefaultCompat:
    ...
    def test_allow_cppstd_helpers_in_package_id(self):
        # https://github.com/conan-io/conan/issues/16147
        # allow valid_max_cppstd() to be used in package_id()
        c = TestClient()
        dep = textwrap.dedent("""
            from conan import ConanFile
            ...
            class Dep(ConanFile):
                ...
                settings = "os", "arch", "compiler", "build_type"
                def package_id(self):
                    cppstd1 = self.info.settings.get_safe('compiler.cppstd')
                    cppstd2 = self.settings.get_safe('compiler.cppstd')
