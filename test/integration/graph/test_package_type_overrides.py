...
liba = textwrap.dedent("""\
    ...
    class LibA(ConanFile):
        ...
        package_type_overrides = {package_type_overrides}
...
@pytest.mark.parametrize("package_type_overrides", [True, False])
def test_package_type_overrides_run_trait(package_type_overrides):
    ...
    c.save({"liba/conanfile.py": liba.format(package_type_overrides=f"{{ 'run': {package_type_overrides} }}"),
            "app/conanfile.py": app.format(requires="liba/0.1")})
    c.run("create liba")

    c.run("build app", assert_error=not package_type_overrides)
    if package_type_overrides:
        assert "MYSHARED RUNTIME!!" in c.out
    else:
        # Without "package_type_overrides" the run trait is False, as it is a static-library, so the
...
@pytest.mark.parametrize("package_type_overrides", [True, False])
def test_package_type_overrides_run_transitive(package_type_overrides):
    ...
    if package_type_overrides:
        ...
        # Without "package_type_overrides" the run trait is False, and as liba is a static-library
