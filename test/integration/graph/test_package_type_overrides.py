...
liba = textwrap.dedent("""\
    ...
    class LibA(ConanFile):
        ...
        package_type_traits = {package_type_traits}
...
def test_package_type_traits_run_trait(package_type_traits):
    ...
    c.save({"liba/conanfile.py": liba.format(package_type_traits=f"{{ 'run': {package_type_traits} }}"),
            "app/conanfile.py": app.format(requires="liba/0.1")})
    c.run("create liba")

    c.run("build app", assert_error=not package_type_traits)
    if package_type_traits:
        assert "MYSHARED RUNTIME!!" in c.out
    else:
        # Without "package_type_traits" the run trait is False, as it is a static-library, so the
...
@pytest.mark.parametrize("package_type_traits", [True, False])
def test_package_type_traits_run_transitive(package_type_traits):
