...
class TestHooks:
    ...
    def test_complete_hook(self):
        ...
        c.run("export . ")
        assert f"pre_export():\n  Hello" in c.out
        assert f"post_export():\n  Hello" in c.out

        c.run("export-pkg . ")
        assert f"{hook_msg} pre_export():\n  Hello" in c.out
        assert f"{hook_msg} post_export():\n  Hello" in c.out
        ...
        assert f"pkg/0.1: {hook_msg} pre_export():\n  Hello" in c.out
        assert f"pkg/0.1: {hook_msg} post_export():\n  Hello" in c.out
    ...
    def test_import_hook(self):
        ...
        c.run("export . ")
        assert "[HOOK - my_hook/hook_my_hook.py] pre_export():\n  my_printer(): CUSTOM MODULE" \
               in c.out
