...
def test_locking_require_override():
    """
    conanfile.txt locking it dependencies and using a ``--require-override``
    that forces an override over a transitive dependency, without version ranges.
    Then, when the lockfile is applied, it will fail, because the conanfile not overriden
    dependency doesn't match the locked one
    """
    client = TestClient()
    client.save({"pkg/conanfile.py": GenConanfile(),
                 "consumer/conanfile.txt": f"[requires]\npkg/0.1"})
