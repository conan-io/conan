...
class TestRequester:
    ...
    def test_requester_timeout(self):
        ...
        client.save({"global.conf": "core.requests:timeout=4.3"}, path=client.cache.cache_folder)
    ...
    def test_requester_timeout_tuple(self):
        client = TestClient(requester_class=MyRequester)
        client.save({"global.conf": "core.requests:timeout=(2, 3.4)"}, path=client.cache.cache_folder)
        client.save({"conanfile.py": conanfile})
        client.run("create . foo/1.0@")
    ...
    def test_no_request_timeout(self):
        ...
        client = TestClient(requester_class=MyRequester)
        client.save({"global.conf": "core.requests:timeout=None"}, path=client.cache.cache_folder)
        client.save({"conanfile.py": conanfile})
        client.run("create . foo/1.0@")
