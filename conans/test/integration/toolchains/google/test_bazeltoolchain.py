...
def test_default_bazel_toolchain(conanfile):
    ...
    content = load(c, os.path.join(c.current_folder, "bazel-conan-tools", BazelToolchain.bazelrc_name))
    ...
    assert "build:conan-config --compilation_mode=fastbuild" in content
