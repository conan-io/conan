...
@pytest.fixture()
def ref():
    return ConanFileReference.loads("pkg/0.1@lasote/stable")


def test_download_recipe(ref):
...
def test_download_with_sources(ref):
    server = TestServer()
    servers = OrderedDict()
...
def test_download_reference_with_packages(ref):
    server = TestServer()
    servers = {"default": server}
...
def test_download_wrong_id(ref):
...
def test_download_full_reference(ref):
    server = TestServer()
    servers = {"default": server}
...
def test_download_package_argument(ref):
    server = TestServer()
    servers = {"default": server}
...
def test_download_not_found_reference(ref):
    server = TestServer()
    servers = {"default": server}
    client = TurboTestClient(servers=servers, users={"default": [("lasote", "mypass")]})
