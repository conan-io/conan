...
def test_inherited_baseclass_append_sys():
    ...
    conanfile_pkg = textwrap.dedent("""
        ...
        sys.path.append(os.getenv('BASE_LOCATION'))
        import conanfile_base
    ...
    with environment_append({"BASE_LOCATION": os.path.join(c.current_folder, "base")}):
        c.run("export pkg1")
        c.run("export pkg2")
        c.run("install app --build=missing")

    base_location = f"{c.current_folder}/base/conanfile_base.py"

    assert f"pkg1/0.1: build of pkg1" in c.out
    assert f"pkg1/0.1: package_info of pkg1" in c.out
    assert f"pkg1/0.1: using conanfile_base {base_location}" in c.out

    assert f"pkg2/0.1: package_info of pkg2" in c.out
    assert f"pkg2/0.1: build of pkg2" in c.out
    assert f"pkg2/0.1: using conanfile_base {base_location}" in c.out

    assert f"conanfile.py (app/0.1): using conanfile_base {base_location}" in c.out


def test_inherited_baseclass_importlib():
    ...
    conanfile_pkg = textwrap.dedent("""
        ...
        spec = spec_from_file_location("conanfile_base", f"{os.getenv('BASE_LOCATION')}/conanfile_base.py")
