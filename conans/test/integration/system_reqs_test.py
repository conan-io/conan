...
class SystemReqsTest(unittest.TestCase):
    ...
    def per_package_test(self):
        ...
        self.assertFalse(os.path.exists(client.client_cache.system_reqs(ref)))
        pref = PackageReference(ref, "f0ba3ca2c218df4a877080ba99b65834b9413798")
        load_file = load(client.client_cache.system_reqs_package(pref))
        ...
        load_file = load(client.client_cache.system_reqs_package(pref2))
        self.assertIn("Installed my stuff", load_file)

        # remove packages
        client.run("remove Test* -f -p 544")
        self.assertTrue(os.path.exists(client.client_cache.system_reqs_package(pref)))
        client.run("remove Test* -f -p f0ba3ca2c218df4a877080ba99b65834b9413798")
        self.assertFalse(os.path.exists(client.client_cache.system_reqs_package(pref)))
        self.assertTrue(os.path.exists(client.client_cache.system_reqs_package(pref2)))
        client.run("remove Test* -f -p %s" % NO_SETTINGS_PACKAGE_ID)
        self.assertFalse(os.path.exists(client.client_cache.system_reqs_package(pref)))
        self.assertFalse(os.path.exists(client.client_cache.system_reqs_package(pref2)))
    ...
    def global_test(self):
        ...
        self.assertFalse(os.path.exists(client.client_cache.system_reqs_package(pref)))
        load_file = load(client.client_cache.system_reqs(ref))
        self.assertIn("Installed my stuff", load_file)
