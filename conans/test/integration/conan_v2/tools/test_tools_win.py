...
class ToolsWinTestCase(ConanV2ModeTestCase):
    def test_msvc_build_command(self):
        with self.assertRaisesRegex(ConanV2Exception, "Conan v2 incompatible: 'tools.msvc_build_command' is deprecated"):
            tools.msvc_build_command(settings=None, sln_path=None)

    def test_build_sln_command(self):
        with self.assertRaisesRegex(ConanV2Exception, "Conan v2 incompatible: 'tools.build_sln_command' is deprecated"):
