...
class CppStdMinimumCheckTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client.save({"conanfile.py": CppStdMinimumCheckTests.CONANFILE})

    @parameterized.expand(["17", "gnu17"])
    def test_cppstd_from_settings(self, cppstd):
        profile = CppStdMinimumCheckTests.PROFILE.replace("{}", "compiler.cppstd=%s" % cppstd)
