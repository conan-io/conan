...
class FixSymlinksTestCase(unittest.TestCase):
    ...
    conanfile = textwrap.dedent("""
        ...
        class Recipe(ConanFile):
            ...
            def build(self):
                tools.save(os.path.join(self.layout.build_folder, "build.txt"), "contents")

            def package(self):
                os.symlink('/dev/null', os.path.join(self.layout.package_folder, "black_hole"))

                # Files: Symlink to file outside the package
                os.symlink(os.path.join(self.layout.build_folder, "build.txt"),
                           os.path.join(self.layout.package_folder, "outside_symlink.txt"))

                # Files: A regular file with symlinks to it
                tools.save(os.path.join(self.layout.package_folder, "regular.txt"), "contents")
                os.symlink(os.path.join(self.layout.package_folder, "regular.txt"),
                           os.path.join(self.layout.package_folder, "absolute_symlink.txt"))
                os.symlink("regular.txt", os.path.join(self.layout.package_folder, "relative_symlink.txt"))

                # Files: A broken symlink
                tools.save(os.path.join(self.layout.package_folder, "file.txt"), "contents")
                os.symlink(os.path.join(self.layout.package_folder, "file.txt"),
                           os.path.join(self.layout.package_folder, "broken.txt"))
                os.unlink(os.path.join(self.layout.package_folder, "file.txt"))

                # Folder: Symlink outside package
                os.symlink(self.layout.build_folder, os.path.join(self.layout.package_folder,
                                                                  "outside_folder"))

                # Folder: a regular folder and symlinks to it
                tools.save(os.path.join(self.layout.package_folder, "folder", "file.txt"), "contents")
                os.symlink(os.path.join(self.layout.package_folder, "folder"),
                           os.path.join(self.layout.package_folder, "absolute"))
                os.symlink("folder", os.path.join(self.layout.package_folder, "relative"))

                # Folder: broken symlink
                tools.save(os.path.join(self.layout.package_folder, "tmp", "file.txt"), "contents")
                os.symlink(os.path.join(self.layout.package_folder, "tmp"),
                           os.path.join(self.layout.package_folder, "broken_folder"))
                shutil.rmtree(os.path.join(self.layout.package_folder, "tmp"))

                #
                os.symlink(os.path.join(self.layout.package_folder, "folder", "file.txt"),
                           os.path.join(self.layout.package_folder, "abs_to_file_in_folder.txt"))
