...
class RemoveCredentials(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client.run_command("git remote add origin https://url.to.be.sustituted", cwd=self.path)

    def test_https(self):
        expected_url = 'https://myrepo.com.git'
        origin_url = 'https://username:password@myrepo.com.git'

        self.client.run_command("git remote set-url origin {}".format(origin_url), cwd=self.path)
