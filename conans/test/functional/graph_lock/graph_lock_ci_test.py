...
class GraphLockCITest(unittest.TestCase):
    ...
    def test(self):
        ...
        clientb.save({"conanfile.py": str(TestConanFile("PkgB", "0.1",
                                                        requires=['PkgA/0.1@user/channel'],
                                                        info=True)),
                      "graph_info.json": lock_file})
        clientb.run("create . PkgB/0.1@user/channel --lock")
        lock_fileb = load(os.path.join(clientb.current_folder, "graph_info.json"))
        ...
        client.save({"graph_info.json": lock_fileb})
