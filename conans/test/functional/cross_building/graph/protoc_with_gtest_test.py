...
class ProtocWithGTestExample(ClassicProtocExampleBase):
    ...
    def test_not_crossbuilding(self):
        """ Build_requires using 'context="build"', but no profile for build_machine is provided """
        profile_host = Profile()
        profile_host.settings["os"] = "Host"
        profile_host.process_settings(self.cache)

        exc_msg = "Package 'application/testing@user/channel' requires 'protoc' on context build," \
                  " but no profile for build_machine has been provided"
        with six.assertRaisesRegex(self, ConanException, exc_msg):
            self._build_graph(profile_host=profile_host, profile_build=None)
