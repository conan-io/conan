...
class CheckCppstdTests(unittest.TestCase):
    ...
    def test_user_inputs(self):
        ...
        with self.assertRaises(ConanException) as raises:
            check_cppstd(conanfile, min="abcdefg")
        self.assertEqual("min parameter must be a number", str(raises.exception))

        with self.assertRaises(ConanException) as raises:
            check_cppstd(conanfile, max="abcdefg")
        self.assertEqual("max parameter must be a number", str(raises.exception))

        with self.assertRaises(ConanException) as raises:
            check_cppstd(conanfile, min="20", max="11")
        self.assertEqual("min parameter is bigger than the max parameter",
    ...
    def test_check_valid_ranges(self, cppstd):
        ...
        check_cppstd(conanfile, min="98")
        check_cppstd(conanfile, min="98", max="17")
        check_cppstd(conanfile, max="17")
        check_cppstd(conanfile, excludes=["20"])
        check_cppstd(conanfile, min="98", max="17", excludes=["20"])
    ...
    def test_check_invalid_ranges(self, cppstd):
        ...
        with self.assertRaises(ConanInvalidConfiguration) as raises:
            check_cppstd(conanfile, min="11", max="20", excludes=["11", "14", "17"])
        self.assertIn("Current cppstd ({}) is excluded from requirements".format(
            cppstd), str(raises.exception))
    ...
    def test_check_gnu_extension(self, cppstd):
        ...
        check_cppstd(conanfile, min=cppstd, gnu_extensions=True)
    ...
    def test_check_compiler_strict(self):
        ...
        conanfile = self._create_conanfile("something_else", "5", "Linux", None, "libstdc++")
        with self.assertRaises(ConanInvalidConfiguration) as raises:
            check_cppstd(conanfile, min="14", strict=True)
        self.assertEqual(
