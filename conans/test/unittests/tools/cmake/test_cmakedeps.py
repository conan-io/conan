...
def test_cmake_deps_links_flags():
    ...
    cpp_info.exelinkflags = ["-OPT:NOICF", "-s FULL_ES3=1"]
    ...
    with mock.patch('conans.ConanFile.dependencies', new_callable=mock.PropertyMock) as mock_deps:
        ...
        assert 'set(mypkg_EXE_LINK_FLAGS_RELEASE "-OPT:NOICF;-s FULL_ES3=1")' in data_cmake
...
def test_component_cmake_deps_links_flags():
    # https://github.com/conan-io/conan/issues/8703
    conanfile = ConanFile(Mock(), None)
    conanfile._conan_node = Mock()
    conanfile._conan_node.context = "host"
    ...
    cpp_info = CppInfo("mypkg", "dummy_root_folder1")
    # https://github.com/conan-io/conan/issues/8811 regression, fix with explicit - instead of /
    cpp_info.components['cmp1'].sharedlinkflags = ["-NODEFAULTLIB", "-OTHERFLAG"]
    cpp_info.components['cmp1'].exelinkflags = ["-OPT:NOICF", "-s FULL_ES3=1"]
    cpp_info.components['cmp1'].objects = ["myobject.o"]
    conanfile_dep = ConanFile(Mock(), None)
    conanfile_dep.cpp_info = cpp_info
    conanfile_dep._conan_node = Mock()
    conanfile_dep._conan_node.ref = ConanFileReference.loads("mypkg/1.0")
    ...
    with mock.patch('conans.ConanFile.dependencies', new_callable=mock.PropertyMock) as mock_deps:
        ...
        files = cmakedeps.content
        data_cmake = files["mypkg-release-x86-data.cmake"]
        assert 'set(mypkg_mypkg_cmp1_SHARED_LINK_FLAGS_RELEASE "-NODEFAULTLIB;-OTHERFLAG")' in data_cmake
        assert 'set(mypkg_mypkg_cmp1_EXE_LINK_FLAGS_RELEASE "-OPT:NOICF;-s FULL_ES3=1")' in data_cmake
        assert 'set(mypkg_mypkg_cmp1_OBJECTS_RELEASE "${mypkg_PACKAGE_FOLDER_RELEASE}/myobject.o")' \
               in data_cmake
