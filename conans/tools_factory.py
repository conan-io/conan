...
class ToolsFactory(object):
    ...
    @staticmethod
    def new(requester=None, output=None):
        ...
        # requester in a closure can be used in the following methods
        requester = requester or requests
        output = output or ConanOutput(sys.stdout)
        ...
        class ConanTools(object):
            ...
            def run_in_windows_bash(conanfile, bashcmd, cwd=None):
                ...
                with ConanTools.environment_append(env_vars):
                    curdir = ConanTools.unix_path(cwd or os.path.abspath(os.path.curdir))
                    # Needed to change to that dir inside the bash shell
                    to_run = 'cd "%s" && %s ' % (curdir, bashcmd)
                    custom_bash_path = os.getenv("CONAN_BASH_PATH", "bash")
                    wincmd = '%s --login -c %s' % (custom_bash_path, ConanTools.escape_windows_cmd(to_run))
            ...
            def msvc_build_command(settings, sln_path, targets=None, upgrade_project=True, build_type=None,
                ...
                """
                vcvars = ConanTools.vcvars_command(settings)
                build = ConanTools.build_sln_command(settings, sln_path, targets, upgrade_project, build_type, arch)
            ...
            def unzip(filename, destination=".", keep_permissions=False):
                ...
                if (filename.endswith(".tar.gz") or filename.endswith(".tgz") or
                    ...
                    return ConanTools.untargz(filename, destination)
                import zipfile
                ...
                with zipfile.ZipFile(filename, "r") as z:
                    ...
                    output.info("Unzipping %s, this can take a while" % ConanTools.human_size(uncompress_size))
                    extracted_size = 0
                    if platform.system() == "Windows":
            ...
            def get(url):
                ...
                ConanTools.download(url, filename)
                ConanTools.unzip(filename)
                os.unlink(filename)
            ...
            def check_sha1(file_path, signature):
                ConanTools.check_with_algorithm_sum("sha1", file_path, signature)

            @staticmethod
            def check_md5(file_path, signature):
                ConanTools.check_with_algorithm_sum("md5", file_path, signature)

            @staticmethod
            def check_sha256(file_path, signature):
                ConanTools.check_with_algorithm_sum("sha256", file_path, signature)
            ...
            def cross_building(settings, self_os=None, self_arch=None):
                ...
                self_arch = self_arch or ConanTools.detected_architecture()
            ...
            class SystemPackageTool(object):
                ...
                def __init__(self, runner=None, os_info=None, tool=None):
                    ...
                    os_info = os_info or ConanTools.OSInfo()
                ...
                def _create_tool(self, os_info):
                    if os_info.with_apt:
                        return ConanTools.AptTool()
                    elif os_info.with_yum:
                        return ConanTools.YumTool()
                    elif os_info.is_macos:
                        return ConanTools.BrewTool()
                    else:
                        return ConanTools.NullTool()
                ...
                def _installed(self, packages):
                    for pkg in packages:
                        if self._tool.installed(pkg):
                            output.info("Package already installed: %s" % pkg)
                            return True
                    return False
            ...
            class NullTool(object):
                ...
                def install(self, package_name):
                    output.warn("Only available for linux with apt-get or yum or OSx with brew")
            ...
            class AptTool(object):
                def update(self):
                    ConanTools._run(self._runner, "%sapt-get update" % self._sudo_str)

                def install(self, package_name):
                    ConanTools._run(self._runner, "%sapt-get install -y %s" % (self._sudo_str, package_name))
            ...
            class YumTool(object):
                def update(self):
                    ConanTools._run(self._runner, "%syum check-update" % self._sudo_str)

                def install(self, package_name):
                    ConanTools._run(self._runner, "%syum install -y %s" % (self._sudo_str, package_name))
            ...
            class BrewTool(object):
                def update(self):
                    ConanTools._run(self._runner, "brew update")

                def install(self, package_name):
                    ConanTools._run(self._runner, "brew install %s" % package_name)
