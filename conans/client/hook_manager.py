...
class HookManager:
    ...
    def __init__(self, hooks_folder):
        self._hooks_folders = hooks_folder
        self.hooks = {}
    ...
    def _load_hooks(self):
        hooks = {}
        for hook_folder in self._hooks_folders:
            for root, dirs, files in os.walk(hook_folder):
                for f in files:
                    if f.startswith("hook_") and f.endswith(".py"):
                        hook_path = os.path.join(root, f)
                        name = os.path.relpath(hook_path, hook_folder).replace("\\", "/")
