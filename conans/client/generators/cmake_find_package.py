...
class CMakeFindPackageGenerator(Generator):
    ...
    find_template = textwrap.dedent("""
        ...
        find_package_handle_standard_args({filename} REQUIRED_VARS
                                          {filename}_VERSION VERSION_VAR {name}_VERSION)
    ...
    find_components_tpl = Template(textwrap.dedent("""\
        ...
        set({{ pkg_filename }}_COMPONENTS {{ pkg_components }})
        ...
        if({{ pkg_name }}_FIND_COMPONENTS)
            foreach(_FIND_COMPONENT {{ '${'+pkg_filename+'_FIND_COMPONENTS}' }})
                list(FIND {{ pkg_filename }}_COMPONENTS "{{ pkg_name }}::${_FIND_COMPONENT}" _index)
                if(${_index} EQUAL -1)
                    conan_message(FATAL_ERROR "Conan: Component '${_FIND_COMPONENT}' NOT found in package '{{ pkg_filename }}'")
                else()
                    conan_message(STATUS "Conan: Component '${_FIND_COMPONENT}' found in package '{{ pkg_filename }}'")
                endif()
            endforeach()
        endif()
