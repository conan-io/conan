...
class CMakeFindPackageGenerator(Generator):
    ...
    find_components_tpl = Template(textwrap.dedent("""\
        ...
        if(NOT {{ public_dep["filename"] }}_FOUND)
            find_dependency({{ public_dep["filename"] }} REQUIRED)
        else()
            conan_message(STATUS "Conan: Dependency {{ public_dep["filename"] }} already found")
        endif()
    ...
    def _find_for_dep(self, pkg_name, pkg_findname, pkg_filename, cpp_info):
        ...
        pkg_public_deps = [
            {
                ...
                "filename": self._get_filename(self.deps_build_info[public_dep])
            }
            for public_dep in cpp_info.public_deps
        ]
        pkg_public_deps_names = ";".join(["{n}::{n}".format(n=n['name']) for n in pkg_public_deps])
        if cpp_info.components:
            ...
            return self.find_template.format(name=pkg_findname, version=pkg_version,
                                             ...
                                             set_found_filename_uppercase=set_found_filename_uppercase,
