...
function(check_compiler_version)
    ...
    elseif(CONAN_COMPILER STREQUAL "gcc")
        ...
        if(${CONAN_COMPILER_VERSION} VERSION_GREATER 4.9)
            message(STATUS "COMPILER GCC>=5, checking major version ${CONAN_COMPILER_VERSION}")
            conan_split_version(${CONAN_COMPILER_VERSION} CONAN_COMPILER_MAJOR CONAN_COMPILER_MINOR)
            message(STATUS "VERSION MINOR ${CONAN_COMPILER_MINOR}")
            if("${CONAN_COMPILER_MINOR}" STREQUAL "")
                message(STATUS "THERE IS NO MINOR!")
                set(_CHECK_VERSION ${VERSION_MAJOR})
            endif()
        endif()
        message(STATUS "VERSION TO CHECK ${_CHECK_VERSION}")
