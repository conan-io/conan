...
conan_package_components_targets("${{{name}_LIBRARY_LIST{build_type_suffix}}}"   # libraries
                                 "${{{name}_LIB_DIRS{build_type_suffix}}}"       # package_libdir
                                 "${{_{name}_DEPENDENCIES{build_type_suffix}}}"  # deps
                                 {name}_LIBRARIES{build_type_suffix}             # out_libraries
                                 {name}_LIBRARIES_TARGETS{build_type_suffix}     # out_libraries_targets
...
class CMakeFindPackageCommonMacros:
    ...
    conan_package_components_targets = textwrap.dedent("""
        function(conan_package_components_targets libraries package_libdir deps out_libraries out_libraries_target build_type package_name component_name)
            unset(_CONAN_ACTUAL_TARGETS CACHE)
            unset(_CONAN_FOUND_SYSTEM_LIBS CACHE)
            foreach(_LIBRARY_NAME ${libraries})
                find_library(CONAN_FOUND_LIBRARY NAME ${_LIBRARY_NAME} PATHS ${package_libdir}
            ...
            if(NOT ${CMAKE_VERSION} VERSION_LESS "3.0")
                # Add all dependencies to all targets
                string(REPLACE " " ";" deps_list "${deps}")
                list(APPEND _out_libraries "${_CONAN_FOUND_SYSTEM_LIBS};${deps_list}")
            endif()
