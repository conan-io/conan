...
class _ProfileValueParser(object):
    ...
    def get_profile(profile_text, base_profile=None):
        doc = ConfigParser(profile_text, allowed_fields=["build_tool_requires",
                                                         "settings", "env",
        ...
        build_tool_requires = _ProfileValueParser._parse_build_tool_requires(doc)
        ...
        for pattern, refs in build_tool_requires.items():
            base_profile.build_tool_requires.setdefault(pattern, []).extend(refs)
    ...
    @staticmethod
    def _parse_build_tool_requires(doc):
        result = OrderedDict()
        if doc.build_tool_requires:
            # FIXME CHECKS OF DUPLICATED?
            for br_line in doc.build_tool_requires.splitlines():
