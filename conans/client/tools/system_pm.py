...
class SystemPackageTool(object):
    ...
    def __init__(self, conanfile, recommends=False, tool=None, default_mode="enabled"):
        self._conanfile = conanfile
        self._is_up_to_date = False
        self._tool = tool or self._create_tool(recommends)
        self._default_mode = default_mode

    def _create_tool(self, recommends):
        os_info = OSInfo()
        ...
        return tool(self._conanfile, recommends)
    ...
    def update(self):
        ...
        if mode in ("disabled", "verify"):
            self._conanfile.output.info("Not updating system_requirements. "
                                        "CONAN_SYSREQUIRES_MODE=%s" % mode)
            return
    ...
    def install(self, packages, update=True, force=False, arch_names=None):
        ...
        if mode in ("verify", "disabled"):
            ...
            if mode == "disabled":
                self._conanfile.output.info("The following packages need to be installed:\n %s"
                                            % "\n".join(packages))
                return
    ...
    def _installed(self, packages):
        ...
        for pkg in packages:
            if self._tool.installed(pkg):
                self._conanfile.output.info("Package already installed: %s" % pkg)
                return True
        return False
...
class BaseTool(object):
    def __init__(self, conanfile, recommends=True):
        self._conanfile = conanfile
        self._recommends = recommends
        self._sudo_str = _get_sudo_str()

    def _run(self, command, accepted_returns=None):
        accepted_returns = accepted_returns or [0, ]
        self._conanfile.output.info("Running: %s" % command)
        if self._conanfile.run(command, True) not in accepted_returns:
...
class NullTool(BaseTool):
    ...
    def install(self, package_name):
        self._conanfile.output.warning("Only available for linux with apt-get, yum, or pacman or OSX"
                                       "with brew or FreeBSD with pkg or Solaris with pkgutil")
...
class AptTool(BaseTool):
    def add_repository(self, repository, repo_key=None):
        if repo_key:
            self._run("wget -qO - %s | %sapt-key add -" % (repo_key, self._sudo_str))
        self._run("%sapt-add-repository %s" % (self._sudo_str, repository))

    def update(self):
        self._run("%sapt-get update" % self._sudo_str)

    def install(self, package_name):
        recommends_str = '' if self._recommends else '--no-install-recommends '
        self._run("%sapt-get install -y %s%s" % (self._sudo_str, recommends_str, package_name))

    def installed(self, package_name):
        exit_code = self._run("dpkg-query -W -f='${Status}' %s | grep -q \"ok installed\""
                                 % package_name, None)
        return exit_code == 0
...
class YumTool(BaseTool):
    ...
    def update(self):
        self._run("%syum check-update -y" % self._sudo_str, accepted_returns=[0, 100])

    def install(self, package_name):
        self._run("%syum install -y %s" % (self._sudo_str, package_name))

    def installed(self, package_name):
        exit_code = self._run("rpm -q %s" % package_name)
        return exit_code == 0
...
class DnfTool(YumTool):
    ...
    def update(self):
        self._run("%sdnf check-update -y" % self._sudo_str, accepted_returns=[0, 100])

    def install(self, package_name):
        self._run("%sdnf install -y %s" % (self._sudo_str, package_name))
...
class PkgTool(BaseTool):
    ...
    def update(self):
        self._run("%spkg update" % self._sudo_str)

    def install(self, package_name):
        self._run("%spkg install -y %s" % (self._sudo_str, package_name))

    def installed(self, package_name):
        exit_code = self._run("pkg info %s" % package_name, None)
        return exit_code == 0
...
class PkgUtilTool(BaseTool):
    ...
    def update(self):
        self._run("%spkgutil --catalog" % self._sudo_str)

    def install(self, package_name):
        self._run("%spkgutil --install --yes %s" % (self._sudo_str, package_name))

    def installed(self, package_name):
        exit_code = self._run('test -n "`pkgutil --list %s`"' % package_name)
        return exit_code == 0
...
class ChocolateyTool(BaseTool):
    ...
    def update(self):
        self._run("choco outdated")

    def install(self, package_name):
        self._run("choco install --yes %s" % package_name)

    def installed(self, package_name):
        exit_code = self._run('choco search --local-only --exact %s | '
...
class PacManTool(BaseTool):
    ...
    def update(self):
        self._run("%spacman -Syyu --noconfirm" % self._sudo_str)

    def install(self, package_name):
        self._run("%spacman -S --noconfirm %s" % (self._sudo_str, package_name))

    def installed(self, package_name):
        exit_code = self._run("pacman -Qi %s" % package_name)
        return exit_code == 0
...
class ZypperTool(BaseTool):
    ...
    def update(self):
        self._run("%szypper --non-interactive ref" % self._sudo_str)

    def install(self, package_name):
        self._run("%szypper --non-interactive in %s" % (self._sudo_str, package_name))

    def installed(self, package_name):
        exit_code = self._run("rpm -q %s" % package_name, None)
        return exit_code == 0
