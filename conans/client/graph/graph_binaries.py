    ...
    def _skip_binaries(graph):
        ...
        while root_nodes:
            ...
            for node in root_nodes:
                ...
                if node.recipe != RECIPE_CONSUMER and node.binary not in (BINARY_BUILD, BINARY_EDITABLE_BUILD, BINARY_EDITABLE):
                    deps_required = set(d.node for d in node.transitive_deps.values() if d.require.files)
                else:
                    deps_required = set(d.node for d in node.transitive_deps.values())
