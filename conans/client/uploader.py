...
class ConanUploader(object):
    ...
    def upload_conan(self, pattern, force=False, all_packages=False, confirm=False,
        ...
        for conan_ref in references:
            ...
            if upload:
                self._upload_conan(conan_ref, force, all_packages, retry, retry_wait, dry_run)

    def _upload_conan(self, conan_ref, force, all_packages, retry, retry_wait, dry_run):
        ...
        self._remote_proxy.upload_conan(conan_ref, retry, retry_wait, dry_run)
        if all_packages:
    ...
    def upload_package(self, package_ref, index=1, total=1, retry=None, retry_wait=None, dry_run=False):
        ...
        self._remote_proxy.upload_package(package_ref, retry, retry_wait, dry_run)
