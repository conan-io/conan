...
def _process_git_repo(repo_url, client_cache, output, runner, tmp_folder, ssl_verify):
    output.info("Trying to clone repo  %s" % repo_url)

    with tools.chdir(tmp_folder):
        runner('git -c http.sslVerify=%s clone "%s" config' % (ssl_verify, repo_url), output=output)
...
def _process_download(item, client_cache, output, tmp_folder, ssl_verify):
    output.info("Trying to download  %s" % _hide_password(item))
    zippath = os.path.join(tmp_folder, "config.zip")
    tools.download(item, zippath, out=output, verify=ssl_verify)
    _process_zip_file(zippath, client_cache, output, tmp_folder, remove=True)


def configuration_install(item, client_cache, output, runner, ssl_verify):
    ...
    try:
        ...
        if item.endswith(".git"):
            ...
            _process_download(item, client_cache, output, tmp_folder, ssl_verify)
        else:
