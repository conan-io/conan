...
def cmd_build(app, conanfile_path, source_folder, build_folder, package_folder, install_folder,
    ...
    try:
        ...
        # FIXME: Ugly & smelly: The -if already sets the whole install path, will be
        #        fixed when the build computes the graph
        conan_file.layout.install.folder = ""
        mkdir(conan_file.layout.build_folder)
        os.chdir(conan_file.layout.build_folder)
